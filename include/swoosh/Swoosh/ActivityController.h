  ...
  public:
    ...
    */
    ActivityController(sf::RenderWindow& window, sf::Vector2u virtualWindowSize) : handle(window) {
      ...
      surface = new sf::RenderTexture();
      surface->resize(virtualWindowSize);
      willLeave = false;
      segueAction = SegueAction::none;
      stackAction = StackAction::none;
  ...
  */
  class CopyWindow : public Activity {
    ...
    void copyWindowContents() {
      ...
      sf::View newView = sf::View(sf::FloatRect({(float)viewportIntRect.position.x, (float)viewportIntRect.position.y}, {(float)viewportIntRect.size.x, (float)viewportIntRect.size.y}));
      ...
      framebuffer.resize({(unsigned int)w, (unsigned int)h});
