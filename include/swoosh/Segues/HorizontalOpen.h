...
public:
 void onDraw(sf::RenderTexture& surface) override {
    ...
    top.setPosition(0.0f, (float)(-alpha * top.getTextureRect().size.y));

    sf::Sprite bottom(temp);
    bottom.setTextureRect(sf::IntRect({0, (int)(windowSize.y / 2.0)}, {windowSize.x, windowSize.y}));
    bottom.setPosition(0.0f, (float)(windowSize.y/2.0f) +  ((float)alpha * (bottom.getTextureRect().size.y-bottom.getTextureRect().position.y)));
