...
public:
 ...
 void onDraw(sf::RenderTexture& surface) override {
    ...
    left.setPosition(sf::Vector2f{(float)(lr * alpha * left.getTexture().getSize().x), (float)(ud * alpha * left.getTexture().getSize().y)});
    ...
    right.setPosition(sf::Vector2f{(float)(-lr * (1.0-alpha) * right.getTexture().getSize().x), (float)(-ud * (1.0-alpha) * right.getTexture().getSize().y)});
